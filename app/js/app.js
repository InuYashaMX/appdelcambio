...
angular.module("myApp")
    .constant("FB_CONFIG", {
        apiKey: "AIzaSyCj2DpfYo1HXDgJPwaYkvuHP_sL7it7do4",
        authDomain: "baches-78ef9.firebaseapp.com",
        databaseURL: "https://baches-78ef9.firebaseio.com",
        storageBucket: "baches-78ef9.appspot.com",
    })
...
angular.module("myApp").controller("chatCtrl", ['$scope', 'F', '$timeout', '$mdDialog', '$location', '$routeParams', '$filter', 'Upload', '$mdToast', function($scope, F, $timeout, $mdDialog, $location, $routeParams, $filter, Upload, $mdToast) {
    ...
    var addChatListener = function(title, roomName) {
        F.db.child("chats/" + $scope.map + "/" + roomName).limitToLast(10).on('child_added', function(snapshot) {
            ...
            if (F.isSuscribe[$scope.map]) {
                ...
                F.db.child("maps").child($routeParams.map).child("users").child("exceptions").child(F.user.key).child("chat").on("value", function(snapshot) {
                    ...
                    if ($scope.chatPrivileges) {
                        ...
                        $scope.showSimpleToast("Estas muteado, puede contactar a un administrador para ayuda.");
                    }
                    $timeout(function() {});
                });
            }
...
angular.module("myApp").controller("proximamenteCtrl", ['$scope', 'F', function($scope, F) {
...
angular.module("myApp").directive('countdown', [
        'Util',
        '$interval',
        function (Util, $interval) {
            return {
                ...
                link: function (scope, element) {
                    var future;
                    future = new Date(String(scope.date));
                    
                    $interval(function () {
                        var diff;
                        diff = Math.floor((future.getTime() - new Date().getTime()) / 1000);
                        console.log(Util.dhms(diff));
...
angular.module("myApp").directive('dynFbCommentBox', function () {
    function createHTML(href, numposts, colorscheme) {
        return '<div class="fb-comments" ' +
                       'data-href="' + href + '" ' +
                       'data-numposts="' + numposts + '" ' +
                       'data-width="150" data-colorsheme="' + colorscheme + '">' +
               '</div>';
    }
...
angular.module("myApp").directive( 'goClick', function ( $location ) {
